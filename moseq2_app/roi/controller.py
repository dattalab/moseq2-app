...
class InteractiveFindRoi(InteractiveROIWidgets):
    ...
    def __init__(self, data_path, config_file, session_config, compute_bgs=True, autodetect_depths=True, overwrite=False):
        ...
        assert len(
            self.sessions) > 0, "No sessions were found in the provided base_dir"

        # Session selection dict key names
        self.keys = list(self.sessions)

        if not os.path.exists(session_config) or overwrite:
            self.session_parameters = {k: detect_and_set_camera_parameters(
                deepcopy(self.config_data), self.sessions[k]) for k in self.keys}
        else:
            self.session_parameters = read_yaml(session_config)
            # add new sessions to config
            for session in set(self.sessions) - set(self.session_parameters):
                self.session_parameters[session] = detect_and_set_camera_parameters(
                    deepcopy(self.config_data), self.sessions[session])
        write_yaml(self.session_parameters, session_config)
        ...
        states = [0] * len(self.sessions)

        c_base = int("1F534", base=16)
        options = list(self.sessions)
        colored_options = ['{} {}'.format(
            chr(c_base + s), o) for s, o in zip(states, options)]
        ...
        self.minmax_heights.value = (self.config_data.get(
            'min_height', 10), self.config_data.get('max_height', 100))
        self.dilate_iters.value = self.config_data.get('dilate_iterations', 0)

        # what do these do? Should they be in config_data?
        ...
        for s, p in tqdm(self.sessions.items(), total=len(self.sessions.keys()), desc='Computing backgrounds'):
            try:
                ...
                if p.endswith('.mkv') and 'timestamps' not in self.session_parameters[s]:
                    ...
                    self.session_parameters[s]['finfo']['nframes'] = len(
                        self.session_parameters[s]['timestamps'])

                # Compute background image; saving the image to a file
                self.session_parameters[s].pop('output_dir', None)
                get_bground_im_file(
                    p, **self.session_parameters[s], output_dir=None)
            except:
                # Print error if an issue arises
                display(
                    f'Error, could not compute background for session: {s}.')
                pass
        ...
        for i, (sessionName, sessionPath) in enumerate(session_dict.items()):
            ...
            if sessionName != self.curr_session:
                ...
                if 'finfo' not in self.session_parameters[sessionName]:
                    self.session_parameters[sessionName]['finfo'] = get_movie_info(
                        sessionPath)
                ...
                bground_im = get_bground_im_file(
                    sessionPath, **self.session_parameters[sessionName])
                try:
                    ...
                    # if roi doesn't fail
                    write_image(join(dirname(sessionPath), 'proc',
                                'roi_00.tiff'), sess_res['roi'])
    ...
    def interactive_find_roi_session_selector(self, session):
        ...
        if 'finfo' not in self.session_parameters[curr_session_key]:
            self.session_parameters[curr_session_key]['finfo'] = get_movie_info(
                self.curr_session)
        ...
        if not isinstance(self.session_parameters[curr_session_key]['bg_roi_depth_range'], str):
            self.bg_roi_depth_range.value = self.session_parameters[
                curr_session_key]['bg_roi_depth_range']
        ...
        self.session_parameters[curr_session_key].pop('output_dir')
        # this step compute bground.tiff
        self.curr_bground_im = get_bground_im_file(
            self.curr_session, **self.session_parameters[curr_session_key])
    ...
    def interactive_depth_finder(self, minmax_heights, fn, dr, di):
        ...
        if self.config_data['autodetect']:
            self.curr_results = self.get_roi_and_depths(
                self.curr_bground_im, self.curr_session)
            ...
            self.session_parameters[curr_session_key]['bg_roi_depth_range'] = (
                int(dr[0]), int(dr[1]))
    ...
    def get_roi_and_depths(self, bground_im, session):
        ...
        if self.config_data['autodetect']:
            ...
            self.session_parameters[curr_session_key]['bg_roi_depth_range'] = (
                bg_roi_range_min, bg_roi_range_max)
        ...
        strel_dilate = select_strel(self.config_data['bg_roi_shape'], tuple(
            self.config_data['bg_roi_dilate']))
        strel_erode = select_strel(self.config_data['bg_roi_shape'], tuple(
            self.config_data['bg_roi_erode']))
        ...
        if self.config_data['use_plane_bground']:
            ...
            self.curr_bground_im = set_bground_to_plane_fit(
                bground_im, plane, join(dirname(session), 'proc'))
    ...
    def get_extraction(self, input_file, bground_im, roi):
        ...
        frame_batches = [range(self.frame_range.value[0],
                               self.frame_range.value[1])]
    ...
    def prepare_data_to_plot(self, roi, minmax_heights, fn):
        ...
        self.session_parameters[curr_session_key]['min_height'] = int(
            minmax_heights[0])
        self.session_parameters[curr_session_key]['max_height'] = int(
            minmax_heights[1])
        ...
        filtered_frames = apply_roi(curr_frame, roi)[0].astype(
            self.config_data['frame_dtype'])

        # filter for included mouse height range
        filtered_frames = threshold_chunk(
            filtered_frames, minmax_heights[0], minmax_heights[1])
        ...
        try:
            ...
            result = {'depth_frames': np.zeros(
                (1, self.config_data['crop_size'][0], self.config_data['crop_size'][1]))}
        ...
        plot_roi_results(self.formatted_key, display_bg, roi,
                         overlay, filtered_frames, result['depth_frames'][0], fn)
        gc.collect()
